...
class UserSerializer(serializers.ModelSerializer):
    role = serializers.CharField(
        source="get_role_display", read_only=False, required=False
    )
    ...
    def validate_role(self, role):
        if role not in ["Developer", "Project Manager"]:
            raise serializers.ValidationError(
                f"User role must be 'Developer' or 'Project Manager', "
    ...
    def update(self, instance, validated_data):
        ...
        instance.mobile_phone = validated_data.get(
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def validate_final_date(self, value):
        if value < timezone.now().date():
            raise serializers.ValidationError(
                f"The final date must be a future date, "

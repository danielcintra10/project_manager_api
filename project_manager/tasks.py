...
def send_email_to_developer_user_after_new_task_created(task_code):
    ...
    try:
        ...
        message = (
            f"Hello, you have a new task assigned to the project {task.project.name}, \n "
        ...
        email_log = EmailLog.objects.create(
            destination_email=task.developer.email,
            email_purpose="C",
            task=task,
            delivered=True,
            error_info=None,
...
def send_email_to_project_manager_if_task_has_reached_its_deadline():
    ...
    for task in tasks:
        try:
            ...
            message = (
                f"The task {task.title} has reached its deadline. \n"
            ...
            recipient_list = [
                task.project.project_manager.email,
            ]
            send_mail(
                subject,
                message,
            ...
            email_log = EmailLog.objects.create(
                destination_email=task.project.project_manager.email,
                email_purpose="F",
                task=task,
                delivered=True,
                error_info=None,

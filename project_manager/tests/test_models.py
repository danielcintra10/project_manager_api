...
class TestProject(TestCase):
    ...
    def test_delete_project(self):
        ...
        self.assertEqual(Project.objects.count(), 1)
        self.assertFalse(
            Project.objects.filter(code__exact=new_project.code).exists()
        )
    ...
    def test_validate_only_project_managers_in_project_manager_field(self):
        with self.assertRaises(ValidationError):
            new_project = Project(
                name="Second Project Created",
                description="Description of the project",
                project_manager=self.test_dev_user,
                created_by=self.test_pm_user,
                updated_by=self.test_pm_user,
            )
...
class TestTask(TestCase):
    ...
    def test_error_when_create_a_task_with_missing_fields(self):
        with self.assertRaises(Exception):
            new_task = Project.objects.create(
                title="Second Project Created",
                developer=self.test_dev_user,
            )
    ...
    def test_delete_task(self):
        ...
        self.assertEqual(Task.objects.count(), 1)
        self.assertFalse(
            Task.objects.filter(code__exact=new_task.code).exists()
        )
    ...
    def test_create_task_with_final_date_before_creation_date_return_validation_error(
        self,
    ):
        with self.assertRaises(ValidationError):
            new_task = Task.objects.create(
...
class TestEmailLog(TestCase):
    ...
    def test_create_new_email_log_with_valid_data(self):
        email_log = EmailLog.objects.create(
            destination_email=self.test_dev_user.email,
            email_purpose="C",
            task=self.test_task,
            delivered=True,
        )

...
class TestProjectSerializer(BaseSerializersClass):
    ...
    def test_deserialize_data_ok(self):
        ...
        self.assertEqual(
            "Project created to test serializers", new_project.description
        )
    ...
    def test_update_instance_with_valid_data(self):
        ...
        serializer = ProjectSerializer(
            instance=project, data=data, context={"request": self.request}
        )
        self.assertTrue(serializer.is_valid(raise_exception=True))
    ...
    def test_relational_field_developer_created_with_invalid_data_is_not_valid(
        self,
    ):
        data = {
            "name": "New Project Created",
...
class TestTaskSerializer(BaseSerializersClass):
    ...
    def test_serialize_with_valid_data(self):
        ...
        self.assertEqual(
            "Task Description", serializer.data.get("description")
        )
        ...
        self.assertEqual(
            date(9999, 10, 10).isoformat(), serializer.data.get("final_date")
        )
        self.assertTrue(serializer.data.get("is_active"))
        self.assertTrue("created_at" in serializer.data)
        self.assertTrue("updated_at" in serializer.data)

    def test_serialize_with_valid_data_without_context_dont_show_project_field(
        self,
    ):
        task = Task.objects.filter(code="test-task-012024").first()
    ...
    def test_deserialize_data_ok(self):
        ...
        serializer = TaskSerializer(
            data=data, context={"request": self.request, "fields": True}
        )
        self.assertTrue(serializer.is_valid(raise_exception=True))
        new_task = serializer.save()
        self.assertEqual("New Task Created", new_task.title)
        self.assertEqual(
            "Task created to test serializers", new_task.description
        )
    ...
    def test_update_instance_with_valid_data(self):
        ...
        serializer = TaskSerializer(
            instance=task,
            data=data,
            context={"request": self.request, "fields": True},
        )
        self.assertTrue(serializer.is_valid(raise_exception=True))
    ...
    def test_relational_field_project_created_with_invalid_data_is_not_valid(
        self,
    ):
        data = {
            "title": "New Task Created",

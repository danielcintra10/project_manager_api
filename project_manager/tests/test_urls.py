...
class TestUrls(TestCase):
    ...
    def test_retrieve_update_destroy_project_url_resolve(self):
        url = reverse(
            "retrieve_update_destroy_project", args=["test-project-012024"]
        )
        self.assertEquals(
            resolve(url).func.view_class, RetrieveUpdateDestroyProject
        )
    ...
    def test_retrieve_update_destroy_task_url_resolve(self):
        url = reverse(
            "retrieve_update_destroy_task", args=["test-task-012024"]
        )
        self.assertEquals(
            resolve(url).func.view_class, RetrieveUpdateDestroyTask
        )

    def test_list_projects_by_project_manager_url_resolve(self):
        url = reverse("list_projects_by_project_manager", args=[2])
        self.assertEquals(
            resolve(url).func.view_class, ListProjectsByProjectManager
        )
    ...
    def test_list_developer_tasks_in_project_url_resolve(self):
        url = reverse(
            "list_developer_tasks_in_project", args=[1, "test-project-012024"]
        )
        self.assertEquals(
            resolve(url).func.view_class, ListDeveloperTasksInProject
        )

    def test_list_available_developers_url_resolve(self):
        url = reverse("list_available_developers")
        self.assertEquals(
            resolve(url).func.view_class, ListAvailableDevelopers
        )
